...
export class AdminPage {
	...
	constructor(readonly page: Page) {
		this.adsLink = this.page.getByTestId("navigationContainer").getByText("Оголошення");
		this.waitingTab = this.page.getByTestId("statusBtns").getByText("Очікуючі");
		this.approveTab = this.page.getByTestId("statusBtns").getByText("Активні");
		this.dataSortColumn = this.page.getByTestId("sortLabelContainer").getByText("Дата створення");
		this.logoutButton = this.page.getByText("Вихід");

		this.servicesLink = this.page.getByTestId("linksContainer").filter({ hasText: "Сервіси" });
	}
	...
	async approveAds(ads: TestAdData[]) {
		...
		for (let i = 0; i < ads.length; i++) {
			...
			const adRecord = this.page.locator("tr", {
				has: this.page.locator("td", { hasText: ads[i].title }),
			});

			await adRecord.getByTestId("adminShowButton").scrollIntoViewIfNeeded();
	...
	async removeService(service: string) {
		...
		const row = this.page.locator("tr").filter({ hasText: service }).filter({ hasText: "Користувацькі" });
	...
	private async confirmDeleting() {
		await this.page.getByTestId("content").getByRole("button", { name: "Так" }).click();
	}

	private async openServicesList() {
		await this.servicesLink.click();
		await this.page.getByText("Список сервісів").waitFor({ state: "visible" });
