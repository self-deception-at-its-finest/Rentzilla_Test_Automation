...
test.describe("Favorite Units Tests", () => {
    ...
    test(
        "The 'Обрані оголошення' page without 'Обрані' units",
        {
            tag: "@UI",
        ...
        async ({ page, authorizedHomePage, favoritePage }) => {
            ...
            await test.step(`2. Click on the 'До списку оголошень' button.`, async () => {
                ...
                await expect(authorizedHomePage.navAdsLink).toHaveClass(
                    FAVORITE_UNITS_CONSTS.CLASSES.NAVBAR_ACTIVE,
                );
            });
        },
    );
    ...
    test(
        ...
        async ({ page, authorizedHomePage, productsPage, favoritePage }) => {
            ...
            await test.step("1-2. Click on the 'Оголошення' button in the header. Click on the add to 'Обрані оголошення' icon of the unit section.", async () => {
                ...
                await expect(
                    heart.locator(
                        `path[fill="${FAVORITE_UNITS_CONSTS.COLORS.RED_FILL}"]`,
                    ),
                ).toBeVisible();
            });
            ...
            await test.step("6. Verify icon state back on Products page", async () => {
                ...
                await expect(
                    heart.locator(
                        `path[stroke="${FAVORITE_UNITS_CONSTS.COLORS.GREY_STROKE}"]`,
                    ),
                ).toBeVisible();
            });
        },
    ...
    test(
        "The 'Очистити список' button functionality",
        {
            tag: "@functional",
        ...
        async ({ page, authorizedHomePage, productsPage, favoritePage }) => {
            ...
            await test.step("4. The 'Так' button works and clears the list", async () => {
                ...
                await expect(favoritePage.emptyDescription).toHaveText(
                    FAVORITE_UNITS_CONSTS.EMPTY_DESCRIPTION,
                );
                await expect(favoritePage.confirmDeleteForm).not.toBeVisible();
            });
    ...
    test(
        "The 'Пошук по назві' search field functionality",
        {
            tag: "@functional",
        ...
        async ({
            ...
            await test.step("2-3. Click on search input and press Enter", async () => {
                ...
                await expect(favoritePage.unitCards).toHaveCount(
                    favoriteUnitsState.length,
                );
            });
            ...
            await test.step("6. Enter '16' in the 'Пошук по назві' input", async () => {
                ...
                if (count > 0) {
                    ...
                    await expect(favoritePage.unitCards.first()).toContainText(
                        query,
                    );
                } else {
                    // if there are no results, check that the empty state message is correct and contains "16"
                    await expect(favoritePage.emptyTitle).toContainText(
                        `Оголошення за назвою "${query}" не знайдені`,
                    );
            ...
            await test.step("7. Enter specific symbols in the 'Пошук по назві' input", async () => {
                ...
                if (count > 0) {
                    ...
                    await expect(favoritePage.emptyTitle).toContainText(
                        `Оголошення за назвою "${symbols}" не знайдені`,
                    );
                    await expect(favoritePage.resetFiltersButton).toBeVisible();
                    await favoritePage.resetFiltersButton.click();
                }
            ...
            await test.step("8. Enter the non-existing keyword 'тест1234567890'", async () => {
                ...
                await expect(favoritePage.emptyTitle).toContainText(
                    `Оголошення за назвою "${nonExistentKey}" не знайдені`,
                );
                await expect(favoritePage.resetFiltersButton).toBeVisible();
                await favoritePage.resetFiltersButton.click();
            });
            ...
            await test.step("9. Search by existing unit title", async () => {
                ...
                const titleFromUI = await favoritePage.unitName
                    .first()
                    .innerText();
                ...
                await expect(favoritePage.unitName.first()).toHaveText(
                    titleFromUI,
                );
            });
        },
    ...
    test(
        "Check the pagination on the 'Обрані оголошення' page",
        {
            tag: "@UI",
    ...
    test(
        "'Всі категорії' dropdown menu functionality",
        {
            tag: "@functional",
        ...
        async ({
            ...
            const subcategoryMap = {
                [SPECIAL_EQUIPMENT.categories["building equipment"].title]:
                    Object.values(
                        SPECIAL_EQUIPMENT.categories["building equipment"]
                            .subcategories,
                    ).map((s) => s.title),
                [SPECIAL_EQUIPMENT.categories["municipal equipment"].title]:
                    Object.values(
                        SPECIAL_EQUIPMENT.categories["municipal equipment"]
                            .subcategories,
                    ).map((s) => s.title),
                [SPECIAL_EQUIPMENT.categories["warehouse equipment"].title]:
                    Object.values(
                        SPECIAL_EQUIPMENT.categories["warehouse equipment"]
                            .subcategories,
                    ).map((s) => s.title),
            };
            ...
            await test.step("--- Navigate to 'Обрані' and verify initial state", async () => {
                ...
                await expect(favoritePage.categoryValue).toHaveText(
                    FAVORITE_UNITS_CONSTS.CATEGORIES.ALL,
                );
            ...
            for (const category of categoriesToCheck) {
                await test.step(`Select '${category}' and verify units`, async () => {
                    ...
                    if (count > 0) {
                        ...
                        const isMatch = expectedSubs.some((sub) =>
                            actualCategoryText.includes(sub),
                        );
                        expect(
                            isMatch,
                            FAVORITE_UNITS_CONSTS.CATEGORIES.NOT_ONE_OF(
                                actualCategoryText,
                                expectedSubs,
                            ),
                        ).toBeTruthy();
                        ...
                        await expect(favoritePage.emptyTitle).toContainText(
                            FAVORITE_UNITS_CONSTS.CATEGORIES.NOT_FOUND(
                                category,
                            ),
                        );
                    }
            ...
            await test.step(`--- Return to 'Всі категорії' and verify state`, async () => {
                await favoritePage.selectCategory(
                    FAVORITE_UNITS_CONSTS.CATEGORIES.ALL,
                );
                await expect(favoritePage.categoryValue).toHaveText(
                    FAVORITE_UNITS_CONSTS.CATEGORIES.ALL,
    ...
    test(
        " 'По даті створення' drop down menu functionality",
        {
            tag: "@functional",
        ...
        async ({
            ...
            await test.step(`1. The 'По даті створення' section is displayed on the menu`, async () => {
                ...
                await expect(favoritePage.sortDropdownValue).toHaveText(
                    new RegExp(FAVORITE_UNITS_CONSTS.SORT_OPTIONS.DATE, "i"),
                );
                const dateStrings =
                    await favoritePage.unitDates.allInnerTexts();
            ...
            await test.step(`2. Select the 'По назві' section, the unit names are displayed in alphabetical order`, async () => {
                await favoritePage.selectSortOption(
                    FAVORITE_UNITS_CONSTS.SORT_OPTIONS.NAME,
                );
                // wait for sorting to be applied because it fails sometimes without waiting
                await page.waitForTimeout(500);
                await expect(favoritePage.sortDropdownValue).toHaveText(
                    new RegExp(FAVORITE_UNITS_CONSTS.SORT_OPTIONS.NAME, "i"),
                );
            ...
            await test.step(`3. Select the 'По даті створення' section, the units are sorted by creation date in descending order`, async () => {
                await favoritePage.selectSortOption(
                    FAVORITE_UNITS_CONSTS.SORT_OPTIONS.DATE,
                );
                await page.waitForTimeout(500);
                const dateStrings =
                    await favoritePage.unitDates.allInnerTexts();
