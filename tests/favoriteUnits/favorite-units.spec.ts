...
test.describe("Favorite Units Tests", () => {
		...
		async ({ userPage: page, authorizedHomePage, productsPage, favoritePage }) => {
			...
			await test.step("5. The previously selected 'Обране' icon on the products list is inactive (not red)", async () => {
				...
				for (const unitName of addedUnits) {
					...
					await expect(
						heart.locator(`path[fill="${FAVORITE_UNITS_CONSTS.COLORS.RED_FILL}"]`),
					).not.toBeVisible();
					await expect(
						heart.locator(`path[stroke="${FAVORITE_UNITS_CONSTS.COLORS.GREY_STROKE}"]`),
					).toBeVisible();
				}
			});
		},
	...
	test(
		...
		async ({ userPage: page, authorizedHomePage, favoriteUnitsState, favoritePage }) => {
			await test.step("1. Navigate to 'Обрані' through Cabinet Sidebar", async () => {
				await authorizedHomePage.navigateToFavoriteAds();
				await expect(page).toHaveURL(FAVORITE_UNITS_CONSTS.URL);
			});
	...
	test(
		...
		async ({ userPage: page, authorizedHomePage, favoritePage, unitPage, favoriteUnitsState: _ }) => {
			...
			const subcategoryMap = {
				[SPECIAL_EQUIPMENT.categories["building equipment"].title]: Object.values(
					SPECIAL_EQUIPMENT.categories["building equipment"].subcategories,
				).map((s) => s.title),
				[SPECIAL_EQUIPMENT.categories["municipal equipment"].title]: Object.values(
					SPECIAL_EQUIPMENT.categories["municipal equipment"].subcategories,
				).map((s) => s.title),
				[SPECIAL_EQUIPMENT.categories["warehouse equipment"].title]: Object.values(
					SPECIAL_EQUIPMENT.categories["warehouse equipment"].subcategories,
				).map((s) => s.title),
			};
			...
			for (const category of categoriesToCheck) {
				await test.step(`Select '${category}' and verify units`, async () => {
					...
					if (count > 0) {
						...
						expect(
							isMatch,
							FAVORITE_UNITS_CONSTS.CATEGORIES.NOT_ONE_OF(actualCategoryText, expectedSubs),
						).toBeTruthy();
						...
						await expect(favoritePage.emptyTitle).toContainText(
							FAVORITE_UNITS_CONSTS.CATEGORIES.NOT_FOUND(category),
						);
					}
				});
			}
	...
	test(
		...
		async ({ userPage: page, authorizedHomePage, favoritePage, favoriteUnitsState: _ }) => {
			await test.step("--- Navigate to 'Обрані' through Cabinet Sidebar", async () => {
				await authorizedHomePage.navigateToFavoriteAds();
				await expect(page).toHaveURL(FAVORITE_UNITS_CONSTS.URL);
			});
