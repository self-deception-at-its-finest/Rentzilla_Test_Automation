...
test.describe(
    ...
    () => {
        test(
            ...
            async ({
                createUnitPageWithFilledFourTabs: _,
                verifiedUserContactsComponent: contacts,
            }) => {
                await test.step("The title contains “Ваші контакти” text", async () => {
                    await expect(contacts.yourContactsTitle).toBeVisible();
                    await expect(contacts.yourContactsTitle).toContainText(
                ...
                await test.step("Contact card has valid data: ⤵️", async () => {
                    await test.step("• Full name", async () => {
                        await expect(contacts.userNameText).toContainText(
                            `${env.user.firstName} ${env.user.lastName}`,
                        );
                    });
                    await test.step("• TIN", async () => {
                        await expect(contacts.tinText).toContainText(
                            tabs.contacts.tinLabel,
                        );
                    });
                    await test.step("• Phone", async () => {
                        await expect(contacts.userPhoneText).toContainText(
                            `${env.user.phone}`,
                        );
                    });
                    await test.step("• Email", async () => {
                        await expect(contacts.userEmailText).toContainText(
                            `${env.user.email}`,
                        );
                    });
                });
        ...
        test(
            ...
            async ({
                ...
                await test.step("The section’s title has the “Контакти оператора” text", async () => {
                    await expect(
                        contacts.contactsOfOperatorTitle,
                    ).toContainText(tabs.contacts.contactsOfOperatorLabel);
                });
                ...
                await test.step("Check box is active and has the “Я оператор технічного засобу” text", async () => {
                    ...
                    await expect(contacts.operatorLabel).toContainText(
                        tabs.contacts.iAmTheOperator,
                    );
                });
                ...
                const operatorNameFields: FieldData[] = [
                    operatorFields[0],
                    operatorFields[1],
                ];
                ...
                await test.step("UI validating of fields: ⤵️", async () => {
                    for (const field of operatorFields)
                        await test.step(`• field’s title is “${field.labelText}”`, async () => {
                            await expect(field.label).toContainText(
                                field.labelText,
                            );
                        });
                    ...
                    await test.step("• fields are empty by default", async () => {
                        for (const field of operatorFields)
                            await expect(field.input).toHaveText("");
                    });
                ...
                await test.step("After clicking on the checkbox, the fields are disappear", async () => {
                    ...
                    for (const field of operatorFields)
                        await expect(field.input).toHaveCount(0);
                });
                    ...
                    await test.step("• The fields cannot contain less than 2 characters", async () => {
                        for (const field of operatorNameFields) {
                            await validateFieldByBothMethods(
                                ...
                                async () => {
                                    await expectFieldError(field.input);
                                    await expect(field.error).toContainText(
                                        tabs.contacts.operatorNameErrors.less2,
                                    );
                                },
                            );
                        }
                        ...
                        for (const field of operatorNameFields) {
                            await validateFieldByBothMethods(
                                ...
                                async () => {
                                    await expectFieldError(field.input);
                                    await expect(field.error).toContainText(
                                        tabs.contacts.operatorNameErrors.more25,
                                    );
                                },
                            );
                        }
                ...
                await test.step("Validate phone input: ⤵️", async () => {
                    ...
                    await test.step("• the number can contain 9 significant digits with correct operator’s codes", async () => {
                        const codes = [
                            50, 66, 95, 99, 67, 68, 96, 97, 98, 63, 73, 93, 91,
                            92, 94,
                        ];
        ...
        test(
            ...
            async ({
                ...
                await test.step("The “Приватна особа” option is choosed by default", async () => {
                    await expect(contacts.customerTypeDropdown).toHaveText(
                        tabs.contacts.userTypes.private,
                    );
                });
                ...
                await test.step("Verify the “РНОКПП (ІПН)” section: ⤵️", async () => {
                    ...
                    await test.step("• the title contains the “РНОКПП (ІПН)” text", async () => {
                        await expect(contacts.customerTinLabel).toContainText(
                            tabs.contacts.tinLabel,
                        );
                    });
                    ...
                    await test.step("• the clue line has the correct text", async () => {
                        await expect(
                            contacts.customerTinDescription,
                        ).toContainText(tabs.contacts.tinDescription);
                    });
                });
                ...
                await test.step("After clicking on the dropdown, the list of types appears", async () => {
                    ...
                    await expect(
                        contacts.customerTypeListContainer,
                    ).toBeVisible();
                    const dropdownItems =
                        await contacts.customerTypeListItems.allTextContents();

                    expect(
                        dropdownItems.every(
                            (item, i) => item === customerTypes[i],
                        ),
                    ).toBeTruthy();
                });
                ...
                await test.step("Checking UI when selecting “ФОП”: ⤵️", async () => {
                    ...
                    await test.step("• it cannot be empty", async () => {
                        ...
                        await expect(
                            contacts.customerTinInputErrorText,
                        ).toBeVisible();
                        await expect(
                            contacts.customerTinInputErrorText,
                        ).toContainText(FIELDS_ERRORS.EMPTY);
                    });
                });
                    ...
                    await test.step("• it cannot have less than 10 digits", async () => {
                        await validateFieldByBothMethods(
                            ...
                            async () => {
                                ...
                                await expect(fopField.error).toHaveText(
                                    tabs.contacts.tinInputErrorText,
                                );
                            },
                        );
                    });
                ...
                await test.step("Checking UI when selecting “Приватна особа”: ⤵️", async () => {
                    await contacts.openCustomerTypeDropdown();
                    await contacts.chooseCustomerType(
                        tabs.contacts.userTypes.private,
                    );
                    ...
                    await test.step("• it cannot have less than 10 digits", async () => {
                        await validateFieldByBothMethods(
                            ...
                            async () => {
                                ...
                                await expect(privateField.error).toHaveText(
                                    tabs.contacts.tinInputErrorText,
                                );
                            },
                        );
                ...
                await test.step("Checking UI when selecting “Юридична особа”: ⤵️", async () => {
                    await contacts.openCustomerTypeDropdown();
                    await contacts.chooseCustomerType(
                        tabs.contacts.userTypes.legal,
                    );

                    await test.step("• the empty “Тип юридичної особи” field appears", async () => {
                        await expect(
                            contacts.legalTypeDropdownLabel,
                        ).toBeVisible();
                        await expect(
                            contacts.legalTypeDropdownLabel,
                        ).toContainText(tabs.contacts.legalTypeLabel);
                    ...
                    await test.step("• the empty “ЄДРПОУ для юридичних осіб” field appears", async () => {
                        ...
                        await expect(contacts.legalNumberLabel).toContainText(
                    ...
                    await test.step("• the empty “Назва юридичної особи” field appears", async () => {
                        ...
                        await expect(contacts.legalNameLabel).toContainText(
                        ...
                        await validateFieldByBothMethods(
                            ...
                            async () => {
                                ...
                                await expect(
                                    ...
                                    tabs.contacts.legalNumberErrorText,
                                );
                            },
                        );
                    });
                    ...
                    await test.step("• it can have less 26 letters / digits / allowed spec. symbols", async () => {
                        for (const pattern of [
                            ...
                            await validateFieldByBothMethods(
                                ...
                                async () => {
                                    await createUnitPage.nextStep();
                                    await expectFieldDefault(
                                        legalFields[1].input,
                                    );
                                    await expect(
                                        legalFields[1].input,
                                    ).toHaveValue(pattern.slice(0, 25));
                                    await expect(
                                        legalFields[1].error,
                                    ).toHaveCount(0);
                                },
