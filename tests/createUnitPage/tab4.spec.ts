...
test.describe(
	...
	() => {
		test(
			...
			async ({ createUnitPageWithFilledThreeTabs: _, priceComponent }) => {
				await test.step("The payment method title: ⤵️", async () => {
					...
					await test.step("• has the “Спосіб оплати” text", async () => {
						await expect(priceComponent.paymentLabel).toContainText(tabs.price.paymentMethodLabel);
					});

					await test.step("• has the asterisk symbol", async () => {
						await expect(priceComponent.paymentRequiredSymbol).toBeVisible();
					});
				});

				await test.step("The “Готівкою / на картку” option is displayed as default", async () => {
					await expect(priceComponent.selectedPaymentMethod).toHaveText(tabs.price.paymentMethods.cash);
				});
				...
				await test.step("After clicking on field dropdown appeared with options: “Готівкою / на картку”, “Безготівковий розрахунок (без ПДВ)”, “Безготівковий розрахунок (з ПДВ)”", async () => {
					...
					const allResults = await priceComponent.allDropdownItems.allTextContents();
				...
				await test.step("After clicking on the variants they are displayed in the field", async () => {
					const methodsCount = await priceComponent.allDropdownItems.count();
					for (let i = 0; i < methodsCount; i++) {
						const chosenVariant = await priceComponent.allDropdownItems.nth(i).innerText();
						await priceComponent.openPaymentMethodDropdown();
						await priceComponent.allDropdownItems.nth(i).click();
						await expect(priceComponent.selectedPaymentMethod).toHaveText(chosenVariant);
					}
				});
			},
		);
			...
			async ({ createUnitPageWithFilledThreeTabs: _, priceComponent }) => {
				await test.step("The price field title: ⤵️", async () => {
					...
					await test.step("• has the “Вартість мінімального замовлення” text", async () => {
						await expect(priceComponent.priceLabel).toContainText(tabs.price.priceFieldLabel);
					});

					await test.step("• has the asterisk symbol", async () => {
						await expect(priceComponent.priceRequiredSymbol).toBeVisible();
					});
				});
				...
				await test.step("Validate the field: ⤵️", async () => {
					...
					await test.step("• its line length limit is 9 digits", async () => {
						...
						await expect(priceComponent.priceField).toHaveValue(validLongPrice);
					...
					await test.step("• it cannot have spaces, letters and special symbols", async () => {
						...
						for (const value of ["123 456", "123456 "]) {
							...
							await expect(priceComponent.priceField).toHaveValue(validValue);
							await priceComponent.clearPriceField();
				...
				await test.step("The currency field has the “UAH” value", async () => {
					await expect(priceComponent.priceCurrencyField).toHaveValue("UAH");
				});
			},
		);
		...
		test(
			...
			async ({ createUnitPageWithFilledThreeTabs: _, priceComponent }) => {
				await test.step("The service price section has the “Вартість Ваших послуг” title", async () => {
					await expect(priceComponent.servicePriceLabel).toBeVisible();

					await expect(priceComponent.servicePriceLabel).toContainText(tabs.price.servicePriceLabel);
				});
				...
				await test.step("The clue line contains “За бажанням Ви можете додати вартість конкретних послуг, які надає технічний засіб”", async () => {
					...
					await expect(priceComponent.servicePriceDesc).toContainText(tabs.price.servicePriceDescription);
				});

				await test.step("The “Додати вартість” button: ⤵️", async () => {
					const addServicePriceButton = priceComponent.getAddServicePriceButton();

					await test.step("• has the “+” icon", async () => {
						await expect(priceComponent.getPathElement(addServicePriceButton)).toHaveAttribute(
							"d",
							SELECT_ICON,
						);
					});
					...
					await test.step("• disappears after clicking on it; input field and select dropdown are appeared", async () => {
						...
						await expect(priceComponent.servicePriceField).toBeVisible();
						await expect(priceComponent.servicePriceCurrencyField).toBeVisible();
						await expect(priceComponent.servicePriceTypeDropdown).toBeVisible();
					});
				});
				...
				await test.step("The currency of service field has the “UAH” value", async () => {
					await expect(priceComponent.servicePriceCurrencyField).toHaveValue("UAH");
				});

				await test.step("The “Per unit” dropdown: ⤵️", async () => {
					const perUnitField = priceComponent.servicePriceTypeDropdown;
					...
					await test.step("• has the “arrow down” icon", async () => {
						await expect(priceComponent.getPathElement(perUnitField)).toHaveAttribute("d", DROPDOWN_ICON);
					});
				});
				...
				await test.step("After clicking an option in “Per unit” dropdown, it is displayed as the chosen option", async () => {
					...
					for (let i = 0; i < typesCount; i++) {
						...
						await expect(priceComponent.servicePriceTypeDropdown).toHaveText(chosenOption, {
							ignoreCase: true,
						});
					}
				});
				...
				await test.step("Selecting the “Shift” type reveals a new field", async () => {
					...
					await priceComponent.chooseItem(priceComponent.allDropdownItems.nth(1));
				...
				await test.step("The shift dropdown has: ⤵️", async () => {
					...
					await test.step("• has the “arrow down” icon", async () => {
						await expect(priceComponent.getPathElement(shiftField)).toHaveAttribute("d", DROPDOWN_ICON);
					});
				});
				...
				await test.step("After clicking an option in “Shift” dropdown, it is displayed as the chosen option", async () => {
					...
					for (let i = 0; i < shiftsCount; i++) {
						...
						await expect(priceComponent.serviceShiftDropdown).toHaveText(chosenOption, {
							ignoreCase: true,
						});
					}
				});
				...
				await test.step("After clicking on the delete service price button, the service price is deleted", async () => {
					...
					await expect(priceComponent.getAddServicePriceButton()).toBeVisible();
				});
			},
		);
		...
		test(
			...
			async ({ createUnitPageWithFilledThreeTabs: page, serviceComponent }) => {
				...
				await test.step("The data in the third tab is saved", async () => {
					expect(serviceComponent.selectedServicesSection).toBeVisible();
				});
			},
		);
		...
		test(
			...
			async ({ createUnitPageWithFilledThreeTabs: page, priceComponent }) => {
				await test.step("The button has the correct text", async () => {
				...
				await test.step("After clicking the «Далі» button the color of the price field and the field’s error message is red if user didn’t set the price", async () => {
					await page.nextStep();
					await expectFieldError(priceComponent.getPriceFieldWrapper());
