...
test.describe("My Ads User2", () => {
	test(
		"The 'Мої оголошення' page without any created units.", // Precondition: the user has no created ads
		{
			tag: "@UI",
...
test.describe("My Ads Standard User", () => {
	...
	const subcategoryMap = {
		[MY_ADS_CONSTS.CATEGORIES.BUILDING_EQUIPMENT]: extractAllTitles(
			SPECIAL_EQUIPMENT.categories["building equipment"],
		),
		[MY_ADS_CONSTS.CATEGORIES.COMMUNAL_EQUIPMENT]: extractAllTitles(
			SPECIAL_EQUIPMENT.categories["municipal equipment"],
		),
		[MY_ADS_CONSTS.CATEGORIES.WAREHOUSE_EQUIPMENT]: extractAllTitles(
					...
					for (const tabName of tabs) {
						await test.step(`Verify category ${category} in tab ${tabName}`, async () => {
							...
							if (await myAdsPage.isCardsVisible()) {
								...
								await expect(myAdsPage.emptyTitle).toContainText(
									`в категорії "${category}" не знайдено`,
								);
							}
						});
					}
					...
					if (isAtLeastTwoUnits) {
						...
						await test.step("2. Select 'По даті створення' sorting and verify chronological order", async () => {
							...
							for (let i = 0; i < dates.length - 1; i++) {
								expect(
									dates[i],
									`Date ${dateStrings[i]} should be >= ${dateStrings[i + 1]}`,
	...
	test(
		"Заголовок оголошення search field functionality",
		{
			tag: "@functional",
			...
			for (const tabName of tabs) {
				await test.step(`--- Вкладка: ${tabName} ---`, async () => {
					...
					await test.step("Case sensitivity check (Case-insensitive)", async () => {
						...
						expect(
							count,
							`The search should be case-insensitive: when searching for "${upperCaseName}", ${count} ads were found`,
						).toBeGreaterThan(0);
					...
					for (const scenario of scenarios) {
						await test.step(`Scenario: ${scenario.label}`, async () => {
							// 10
							await myAdsPage.clearSearch();
					...
					if (tabName !== MY_ADS_CONSTS.TABS.ACTIVE && allUnitNames.length > 0) {
						...
						await test.step(`Verify unit from ${tabName} is NOT in Active tab`, async () => {
							// 11-12
							await myAdsPage.clearSearch();
