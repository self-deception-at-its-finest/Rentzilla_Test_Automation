...
test.describe("Home Page Footer Tests", () => {
    test(
        "Verify footer elements visibility and navigation",
        {
            tag: "@UI",
        ...
        async ({ page, homePage }) => {
            ...
            await test.step("1-11. Visibility: scroll to footer and check all static elements", async () => {
                ...
                await expect
                    .soft(footer.getLabelByText(FOOTER_CONSTS.LABELS.ABOUT_US))
                    .toBeVisible();
                await expect
                    .soft(footer.getLabelByText(FOOTER_CONSTS.LABELS.FOR_USERS))
                    .toBeVisible();
                await expect
                    .soft(footer.getLabelByText(FOOTER_CONSTS.LABELS.CONTACTS))
                    .toBeVisible();
                ...
                const logoTagName = await footer.logo.evaluate(
                    (el) => el.tagName,
                );
                if (logoTagName === "A") {
                ...
                await expect
                    .soft(
                        page.getByRole("heading", {
                            name: FOOTER_CONSTS.EXPECTED_TITLES.PRIVACY_PAGE,
                        }),
                    )
                    .toBeVisible();
            });
            ...
            await test.step("13. Verify 'Правила використання файлів cookie' link", async () => {
                ...
                await expect
                    .soft(
                        page.getByText(
                            FOOTER_CONSTS.EXPECTED_TITLES.COOKIES_PAGE,
                        ),
                    )
                    .toBeVisible();
            });
            ...
            await test.step("14. Verify 'Умови доступу та користування' link", async () => {
                ...
                await expect
                    .soft(
                        page.getByRole("heading", {
                            name: FOOTER_CONSTS.EXPECTED_TITLES.TERMS_PAGE,
                        }),
                    )
                    .toBeVisible();
            });
            ...
            await test.step("15-16. Verify 'Оголошення' link and return to Home", async () => {
                ...
                const adsSearch = page.getByPlaceholder(
                    FOOTER_CONSTS.PLACEHOLDERS.ADS_SEARCH,
                );
                ...
                await expect
                    .soft(
                        page.getByText(
                            FOOTER_CONSTS.EXPECTED_TITLES.MAIN_PAGE_HERO,
                        ),
                    )
                    .toBeVisible();
            });
            ...
            await test.step("17-18. Verify 'Тендери' link and return to Home", async () => {
                ...
                const tendersSearch = page.getByPlaceholder(
                    FOOTER_CONSTS.PLACEHOLDERS.TENDERS_SEARCH,
                );
            ...
            await test.step("18,5. Verify 'Запити на роботу' link and return to Home", async () => {
                await footer.container.scrollIntoViewIfNeeded();
                await footer
                    .getLinkByText(FOOTER_CONSTS.LINKS.REQUESTS)
                    .click();

                await expect.soft(page).toHaveURL(ENDPOINTS.REQUESTS);

                const requestsSearch = page.getByPlaceholder(
                    FOOTER_CONSTS.PLACEHOLDERS.REQUESTS_SEARCH,
                );
            ...
            await test.step("19. Verify Email link", async () => {
                await footer.container.scrollIntoViewIfNeeded();
                await expect
                    .soft(footer.getLabelByText(FOOTER_CONSTS.LINKS.EMAIL))
                    .toBeVisible();
                await expect
                    .soft(footer.emailLink)
                    .toHaveAttribute(
                        "href",
                        `mailto:${FOOTER_CONSTS.LINKS.EMAIL}`,
                    );
            });
        },
    );
