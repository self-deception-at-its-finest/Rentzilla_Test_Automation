...
test.describe("Home Page tests", () => {
	test(
		"Checking 'Послуги' section on the main page",
		{
			tag: "@UI",
			...
			for (let i = 0; i < tabsCount; i++) {
				...
				await test.step(`Tab: «${tabName}»`, async () => {
					...
					expect
						.soft(
							servicesCount,
							`Кількість елементів у табі «${tabName}» має бути 7, знайдено ${servicesCount}`,
						)
						.toBe(MAIN_PAGE_CONSTS.SERVICES.EXPECTED_COUNT);
					...
					if (servicesCount > 0) {
						...
						await test.step("--- Click on the first relevant unit", async () => {
							...
							if (unitsCount > 0) {
								...
								markStepAsSkipped(
									"Selection of a relevant unit",
									"The website currently has no units in this category.",
								);
							}
						});
	...
	test(
		"Checking 'Спецтехніка' section on the main page",
		{
			tag: "@UI",
		...
		async ({ userPage: page, productsPage, homePage }) => {
			await test.step("1. Open home page and scroll to Special Equipment section", async () => {
				...
				await expect(homePage.specialEquipmentTitle).toHaveText(
					MAIN_PAGE_CONSTS.SPECIAL_EQUIPMENT.SECTION_TITLE,
				);
			});
			...
			for (let i = 0; i < tabsCount; i++) {
				...
				await test.step(`Tab: «${tabName}»`, async () => {
					...
					expect
						.soft(itemsCount, `Кількість елементів у табі «${tabName}» має бути 7, знайдено ${itemsCount}`)
						.toBe(MAIN_PAGE_CONSTS.SPECIAL_EQUIPMENT.EXPECTED_COUNT);
