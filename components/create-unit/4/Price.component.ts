...
export class PriceComponent {
	...
	constructor(readonly page: Page) {
		this.requiredMessage = this.page.getByTestId("div_required_RowUnitPrice").first();

		this.priceField = this.page.getByPlaceholder(tabs.price.priceFieldPlaceholder).first();
		this.servicePriceField = this.page.getByPlaceholder(tabs.price.priceFieldPlaceholder).last();

		this.paymentMethodDropdown = this.page.getByTestId("div_CustomSelect").first();
		this.servicePriceTypeDropdown = this.page.getByTestId("div_CustomSelect").nth(1);
		this.serviceShiftDropdown = this.page.getByTestId("div_CustomSelect").last();
		this.selectedPaymentMethod = this.paymentMethodDropdown.locator("> span:first-of-type");
		this.selectedServicePriceType = this.servicePriceTypeDropdown.locator("> span:first-of-type");
		this.selectedShift = this.serviceShiftDropdown.locator("> span:first-of-type");

		this.dropdownItemsContainer = this.page.getByTestId("listItems-customSelect");
		this.allDropdownItems = this.dropdownItemsContainer.getByTestId("item-customSelect");
		...
		this.priceCurrencyField = this.page.locator('input[value="UAH"]').first();
		this.servicePriceCurrencyField = this.page.locator('input[value="UAH"]').last();

		this.servicePriceLabel = this.page.getByTestId("div_servicePrices_PricesUnitFlow");
		this.servicePriceRequiredSymbol = this.servicePriceLabel.locator("span");
		this.servicePriceDesc = this.page.getByText(tabs.price.servicePriceDescription);

		this.selectedServicesSection = this.servicePriceLabel.locator(" ~ div:last-of-type");
	...
	getAddServicePriceButton(service: Locator = this.selectedServicesList.first()) {
	...
	async removeServicePrice() {
		await this.page.getByTestId("div_removePrice_RowUnitPrice").first().click();
	}
